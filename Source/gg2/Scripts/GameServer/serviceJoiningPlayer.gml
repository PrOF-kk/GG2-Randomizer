...
case STATE_CLIENT_AUTHENTICATED:
    ...
    write_ubyte(socket, string_length(global.motd));
    write_string(socket, global.motd);
...
case STATE_EXPECT_NAME:
    ...
    if ds_list_find_index(global.mutelist, socket_remote_ip(socket)) >= 0 player.muted = true;
    else if ds_list_find_index(global.unmutelist, socket_remote_ip(socket)) >= 0 player.muted = false;
    if ds_list_find_index(global.rconlist, socket_remote_ip(socket)) >= 0 player.rcon = true;
