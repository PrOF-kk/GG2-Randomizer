{
    /*LOG
    -added a weapon modifier. Can be used as blacklist
    -fixed the buffbanner :ooooo
    -fixed a little glitch with class limits preventing loadouts to be changed
    -fixed all servers showing up as modded
    -sentries get destroyed on loadout switch
    -ubersaw now actually works (gives 20% uber on hit and 100% on tauntkill)
    -ubersaw tauntheal is now activated with m2.
    -demo tauntkill no longer teamkills
    -flashlight shouldn't cause crashes anymore
    -synced napalm grenades
    -slightly buffed stuckycharm because i'm too lazy to invent a new one
    -udp servers no longer break the lobby
    -you are no longer randomized during arena set-up
    -added support for little update messages
    -added multistab for all knives except big earner and zapper
    LOG since RM 4.0
    -fixed the stucky charm on sentry crash
    -added /execute for hosts
     */  
    ...
    global.randomizerKey = ini_read_string("Haxxy", "SecretRandomizerKey", "");
    ...
    //
    global.randomizer=ini_read_real("Server", "Randomizer", 0);
    global.medieval=ini_read_real("Server", "Medieval", 0);
    global.motd = ini_read_string("Server", "motd", "Welcome to randomizer mod! You are muted by default.");
    global.serverMotd = "";
    global.newMap = "";
    global.dmCaplimit=ini_read_real("Server", "deathmatch kill limit", 50);
    global.mutePass=ini_read_string("Server", "unmute password","");
    global.rconPass=ini_read_string("Server", "rcon password", "");
    global.defaultMute=ini_read_real("Server", "default mute", 1);
    global.saxtonhale=noone;
    global.plugins = false;
    global.chat=ini_read_real("Server", "Chat", 2);
    global.seriousServer=true;
    global.saxtonmode=ini_read_real("Server", "Saxtonmode", 0);
    ...
    //
    ini_write_string("Haxxy", "SecretrandomizerKey", global.randomizerKey);
    ini_write_real("Server", "Randomizer", global.randomizer);
    ini_write_real("Server", "Medieval", global.medieval);
    ini_write_string("Server", "motd", global.motd);
    ini_write_string("Server", "unmute password", global.mutePass);
    ini_write_string("Server", "rcon password", global.rconPass);
    ini_write_real("Server", "default mute", global.defaultMute);
    ini_write_real("Server", "deathmatch kill limit", global.dmCaplimit);
    ini_read_real("Server", "Chat", global.chat);
    ini_read_real("Server", "SeriousServer", global.seriousServer);
    ini_read_real("Server", "Saxtonmode", global.saxtonmode);
...
global.launchMap = "";
    ...
    message_text_font(global.gg2Font,9,c_white,1);
    message_button_font(global.gg2Font,9,c_white,1);
    message_input_font(global.gg2Font,9,c_white,0);
